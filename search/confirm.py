...
# ============ DIRECT-COUNTING CONFIRMATION OF SCREENED SURVIVORS ==========
# The tail fit is a screen, not a verdict: run twice on different hardware it
# disagreed by up to 10 dex in the far tail and on 3 of 10 survivors. Anything
...
ap.add_argument("--out",default="confirm_triples.json")
ap.add_argument("--topn",type=int,default=0)   # 0 = only Bonferroni survivors
A=ap.parse_args()
...
def forms_tri(pa,pb,pc):
    """Identical to the definition in sweepT.py -- must not diverge."""
...
blob=json.load(open(os.path.expanduser("~/"+A.infile)))
R=blob["results"]; ntests=len(R)
alpha=0.05/ntests
floor=1.0/(A.shifts+1)
print("screened tests: %d   Bonferroni alpha = %.3e"%(ntests,alpha))
print("confirmation shifts: %d  -> p-floor %.3e  (%.1fx below alpha)"
      %(A.shifts,floor,alpha/floor),flush=True)
if floor>=alpha:
    print("REFUSING: floor is not below the threshold; raise --shifts"); raise SystemExit(1)

sel=[r for r in R if r["p"]<alpha]
sel.sort(key=lambda r:r["p"])
if A.topn: sel=sel[:A.topn]
print("selected for confirmation: %d\n"%len(sel),flush=True)
if not sel:
    json.dump(dict(screened=ntests,alpha=alpha,selected=0,results=[]),
              open(os.path.expanduser("~/"+A.out),"w"))
    print("nothing survived the screen -- no confirmation needed"); raise SystemExit(0)

idx={n:i for i,n in enumerate(names)}
def parse(c):
    parts=c.split(" / ")
    return [idx[p] for p in parts]
...
def confirm(arg):
    ...
    n=int(ov.sum())
    pa=prep(M[i][ov]); pb=prep(M[j][ov]); pc=prep(M[k][ov])
    ...
    return dict(combo="%s / %s / %s"%(names[i],names[j],names[k]),form=fname,
                obs=float(obs),p_screen=pscr,p_counted=p,exceed=ge,nshift=tot,
                ndays=n,survives=bool(p<alpha))

jobs=[(parse(r["combo"]),r["form"],r["p"]) for r in sel]
t0=time.time()
with Pool(min(A.pool,max(1,len(jobs)))) as pool:
    out=[r for r in pool.map(confirm,jobs) if r]
el=time.time()-t0
print("confirmed %d tests in %.1f min (%.0f evaluations/s)\n"
      %(len(out),el/60,len(out)*A.shifts/max(el,1)),flush=True)
out.sort(key=lambda r:r["p_counted"])
print("  %-52s %-14s %11s %11s %5s"%("combination","form","p screened","p counted","keeps"))
for r in out:
    print("  %-52s %-14s %11.3e %11.3e %5s"
          %(r["combo"][:52],r["form"],r["p_screen"],r["p_counted"],"YES" if r["survives"] else "no"),flush=True)
kept=sum(1 for r in out if r["survives"])
print("\n  CONFIRMED: %d of %d screened survivors hold up under direct counting"%(kept,len(out)),flush=True)
json.dump(dict(screened=ntests,alpha=alpha,shifts=A.shifts,selected=len(sel),
               confirmed=kept,results=out),open(os.path.expanduser("~/"+A.out),"w"),indent=1)
